...
export async function inviteMember(firmId, email, role, byEmail) {
  const r = (['owner','admin','viewer'].includes(role)) ? role : 'admin';
  await query(
    `INSERT INTO admins (email, firm_id, role, status, invited_by, active) VALUES ($1,$2,$3,'active',$4,true)
     ON CONFLICT (email) DO UPDATE SET firm_id=$2, role=$3, status='active', active=true, invited_by=$4`,
    [email.toLowerCase(), firmId, r, byEmail]);
}
